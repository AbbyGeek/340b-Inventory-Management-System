...
def MedFormatting(item, ndc_input):
    ...
    med_dict = dict(ndc_code = ndc_input, generic_name = generic_name, brand_name = brand_name, package_info = pakcage_info, dosage_form = dosage_form, ingredients = ingredient_list, route = route, pharm_class = pharm_class)              
    return med_dict
...
while user_input != 'quit':
    ...
    if user_input != 'quit':
        ...
        if search_item is not None:
            ...
            print("NDC Code: " + med_dict["ndc_code"])
            print("Generic Name: " + med_dict['generic_name'])
            print("Brand Name: " + med_dict['brand_name'])
            print("Package Info: " + med_dict['package_info'])
            print("Dosage Form: " + med_dict['dosage_form'])
            print ("Drug Class: ")
            for med_class in med_dict['pharm_class']:
                print("    - " + med_class)
            print("Route: ")
            for route in med_dict['route']:
                print("    - " + route)
